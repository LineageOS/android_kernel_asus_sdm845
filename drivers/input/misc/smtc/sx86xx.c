...
static irqreturn_t sx86XX_interrupt_thread(int irq, void *data)
{
  ...
  dev_info(this->pdev, "sx86XX_interrupt_thread\n");
  if ((!this->get_nirq_low) || this->get_nirq_low()) {
...
{
	int err = 0;
	dev_info(this->pdev, "enter sx86xxx_init\n");
  if (this && this->pDevice)
  {
    ...
    /* initailize interrupt reporting */
    this->irq_disabled = 0;
	  err = request_irq(this->irq, sx86XX_irq, IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
