...
static ssize_t fts_reset_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    mutex_lock(&input_dev->mutex);
	ret = fts_i2c_write_reg(fts_data->client, FTS_REG_COVER_MODE_EN, 0x01);
	if(ret < 0)
		FTS_ERROR("[reset]write value to 0xC1 fail\n");
	else{
		fts_reset_proc(300);
		retval = fts_i2c_read_reg(fts_data->client, FTS_REG_COVER_MODE_EN, &reg_value);
		if(retval < 0)
			FTS_ERROR("[reset]read value from 0xC1 fail\n");
		else{
			if(reg_value == 0x01){
				FTS_INFO("[reset]reset tp fail");
				count = snprintf(buf,  PAGE_SIZE, "fail\n");
			}
			else if(reg_value == 0x0){
				FTS_INFO("[reset]reset tp pass");
				count = snprintf(buf,  PAGE_SIZE, "pass\n");
			}
			else{
				FTS_INFO("[reset]reset tp NA");
				count = snprintf(buf,  PAGE_SIZE, "reset tp NA\n");
			}
		}
	}
    mutex_unlock(&input_dev->mutex);
...
{
    FTS_INFO("[reset]attempted to write to read-only attribute fts_reset");
    return count;
}
...
static DEVICE_ATTR(fts_reset, S_IRUGO | S_IWUSR, fts_reset_show, fts_reset_store);
